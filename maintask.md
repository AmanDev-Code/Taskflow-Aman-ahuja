...
Mid-level Engineer · Full Stack / Frontend / Backend Estimated time: 3–5 hours · Deadline: 72 hours from receipt
...
Required for: Full Stack and Backend roles Language: Go (preferred). If you're not comfortable with Go, use a language you know well — note your choice in the README.
...
All non-auth endpoints require Authorization: Bearer 
Projects API
Method	Endpoint	Description
...
git clone [https://github.com/your-name/taskflow](https://github.com/your-name/taskflow)
cd taskflow
cp .env.example .env
docker compose up
...
1. What You'd Do With More Time
...
Create a public GitHub repository — name it taskflow-[your-name]
Repo structure — monorepo with /backend and /frontend directories, or two separate repos linked from the README. docker-compose.yml at root.
No secrets in git — commit .env.example, never .env. If you accidentally commit secrets, rotate them before submitting.
Send us the link — reply to the assignment email with your GitHub URL before the deadline.
Expect a code review call — we'll schedule a 30-minute session to walk through your decisions. You should be able to explain any part of your code.
...
// Request
{ "name": "Jane Doe", "email": "[jane@example.com](mailto:jane@example.com)", "password": "secret123" }

// Response 201
{ "token": "", "user": { "id": "uuid", "name": "Jane Doe", "email": "[jane@example.com](mailto:jane@example.com)" } }
POST /auth/login

// Request
{ "email": "[jane@example.com](mailto:jane@example.com)", "password": "secret123" }

// Response 200
{ "token": "", "user": { "id": "uuid", "name": "Jane Doe", "email": "[jane@example.com](mailto:jane@example.com)" } }
Projects endpoints
GET /projects — requires Authorization: Bearer 
